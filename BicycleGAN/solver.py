...
class Solver():
    def __init__(self, root='/home/tmondal/Downloads/edges2shoes/', result_dir='result', weight_dir='weight', load_weight=False,
        ...
        for epoch in range(self.start_epoch, self.num_epoch):
            for iters, (img, ground_truth) in enumerate(self.dloader):
                ...
                if iters % self.save_every == 0:
                    print(
                        '[Epoch : %d / Iters : %d] => D_loss : %f / G_GAN_loss : %f / KL_div : %f / img_recon_loss : '
                        '%f / z_recon_loss : %f' \
                        % (epoch, iters, D_loss.data[0], G_GAN_loss.data[0], KL_div.data[0], img_recon_loss.data[0],
                           G_alone_loss.data[0]))
