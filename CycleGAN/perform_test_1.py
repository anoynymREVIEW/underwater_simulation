...
def main():
    ...
    # Load data
    # test_loader = getTrainingTestingData(batch_size=batch_size)
    test_loader = torch.load('/sanssauvegarde/homes/t20monda/MUNIT/NYU/train_loader.pkl')
...
def ClacAccuracyOnly(test_loader):
    ...
    for i in range(need_train+1, N):
        ...
        saving_path_complex_imag_GT = '/sanssauvegarde/homes/t20monda/CycleGAN_Resources/' + 'Batch_%d' % i + '_Complex_Imag_GT' + '.pt'
        saving_path_complex_imag_Pred = '/sanssauvegarde/homes/t20monda/CycleGAN_Resources/' + 'Batch_%d' % i + '_Complex_Imag_Pred' + '.pt'
...
def LogProgress(test_loader):
    writer_1 = SummaryWriter('/homes/t20monda/CycleGAN/runs/real_cycleGAN_running_test')
    ...
    checkpoint_G_A2B = torch.load('/homes/t20monda/CycleGAN/checkpoint/' + model_name + '/Models' + '_%s' % 'netG_A2B' + '.ckpt')
    checkpoint_G_B2A = torch.load('/homes/t20monda/CycleGAN/checkpoint/' + model_name + '/Models' + '_%s' % 'netG_B2A' + '.ckpt')
    checkpoint_D_A = torch.load('/homes/t20monda/CycleGAN/checkpoint/' + model_name + '/Models' + '_%s' % 'netD_A' + '.ckpt')
    checkpoint_D_B = torch.load('/homes/t20monda/CycleGAN/checkpoint/' + model_name + '/Models' + '_%s' % 'netD_B' + '.ckpt')
    ...
    for i, sample_batched in enumerate(test_loader):
        if cnt_batch > need_train : # to skip the last batch from training  
            ...
            saving_path_complex_imag_GT = '/sanssauvegarde/homes/t20monda/CycleGAN_Resources/' + 'Batch_%d' % i + '_Complex_Imag_GT' + '.pt'
            saving_path_complex_imag_Pred = '/sanssauvegarde/homes/t20monda/CycleGAN_Resources/' + 'Batch_%d' % i + '_Complex_Imag_Pred' + '.pt'
            saving_path_complex_imag_Pred_A = '/sanssauvegarde/homes/t20monda/CycleGAN_Resources/' + 'Batch_%d' % i + '_Complex_Imag_Pred_A' + '.pt'
