...
def main():
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        epoch_loss_D_A = running_batch_losses_D_A / N  # epoch error
        batch_mean_loss_D_A = (np.mean(keep_all_batch_losses_D_A))

        epoch_loss_D_B = running_batch_losses_D_B / N  # epoch error
...
def _save_best_model(model_1, best_loss, epoch, str_to_add):
    ...
    if not os.path.isdir('/homes/t20monda/CycleGAN/checkpoint/' + model_name):
        os.makedirs('/homes/t20monda/CycleGAN/checkpoint/' + model_name)

    torch.save(state,
               '/homes/t20monda/CycleGAN/checkpoint/' +
               model_name + '/Models_Make_3D' + '_%s' % str_to_add + '.ckpt')  
